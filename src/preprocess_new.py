...
def process_new_speakers(data_dir, frames_dir):
    """Process only new speakers (s21+)."""
    data_dir = Path(data_dir)
    frames_dir = Path(frames_dir)
    frames_dir.mkdir(parents=True, exist_ok=True)

    # Sirf s21 onwards speakers select karo
    speakers = sorted([d for d in data_dir.iterdir() if d.is_dir() and d.name.startswith("s")])
    new_speakers = [sp for sp in speakers if int(sp.name[1:]) > 20]
...
if __name__ == "__main__":
    ...
    FRAMES_DIR = r"D:\lip_reading_ai\frames_unseen"  # new unseen frames yaha save honge

    process_new_speakers(DATA_DIR, FRAMES_DIR)
    print("✅ Lip frames extraction complete for new speakers!")
